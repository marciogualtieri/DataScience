...
build_column_popup <- function(data, column, state){
  state_data <- data[data$state == state, ]
  content <- paste0("<h3>", state, " (by ", column, ")</h3>")
  column_values <- unique(state_data[[column]])
  for(column_value in column_values) {
    content <- paste0(content, "<p>", column_value, ": ", state_data$percentage[state_data[[column]] == column_value], "%</p>")
  }
  return(content)
}
...
leaflet(states, width = "100%") %>%
  ...
  add_polygons(states, "candidate_popup", "Tricked ny Candidate") %>%
